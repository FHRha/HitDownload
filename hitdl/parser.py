...
def parse_yandex_music(page_url: str, log_cb=None) -> List[Dict]:
    ...
    match_playlist = re.search(r"music\.yandex\.ru/playlists?/([A-Za-z0-9\-\.]+)", page_url)
    match_track = re.search(r"music\.yandex\.ru/album/\d+/track/(\d+)", page_url)
    match_album = re.search(r"music\.yandex\.ru/album/(\d+)", page_url)

    if match_track:
        track_id = match_track.group(1)
        if log_cb: log_cb(f"Fetching Yandex Music track: {track_id}")
        t = client.tracks([track_id])
        if not t:
            raise Exception("Track not found")
        tracks_data = t
    elif match_album:
        album_id = match_album.group(1)
        if log_cb: log_cb(f"Fetching Yandex Music album: {album_id}")
        album = client.albums_with_tracks(album_id)
        if not album or getattr(album, 'error', None):
            raise Exception("Album not found or inaccessible")
        tracks_data = []
        for vol in album.volumes:
            tracks_data.extend(vol)
    elif match_playlist:
        ...
        raise Exception("Invalid Yandex Music URL. Only playlists, albums, and tracks are supported.")
...
def search_youtube(artist: str, title: str, log_cb=None) -> str:
    ...
    query = f"{artist} - {title}"
    if log_cb:
        log_cb(f"[cyan]Поиск на YouTube: {query}[/cyan]")
    ...
    try:
        ...
        if log_cb: log_cb(f"[yellow]Ошибка поиска YouTube: {e}[/yellow]")
        return None
        
    entries = info.get('entries', [])
    if not entries:
        if log_cb: log_cb(f"[yellow]На YouTube ничего не найдено: {query}[/yellow]")
        return None
    ...
    if best_score > 0.6 and best_url:
        if log_cb: log_cb(f"[green]Найдено на YouTube (score: {best_score:.2f})[/green]")
        return f"youtube://{best_url}"
        
    if log_cb:
        log_cb(f"[yellow]Нет подходящего совпадения на YouTube (лучший score: {best_score:.2f})[/yellow]")
    return None

def search_soundcloud(artist: str, title: str, log_cb=None) -> str:
    import difflib
    import re
    try:
        import yt_dlp
    ...
    query = f"{artist} - {title}"
    if log_cb:
        log_cb(f"[cyan]Поиск на SoundCloud: {query}[/cyan]")
    ...
    try:
        ...
        if log_cb: log_cb(f"[yellow]Ошибка поиска SoundCloud: {e}[/yellow]")
        return None
        
    entries = info.get('entries', [])
    if not entries:
        if log_cb: log_cb(f"[yellow]На SoundCloud ничего не найдено: {query}[/yellow]")
        return None
    ...
    for entry in entries:
        ...
        score = similarity(query, sc_title)
        if score > best_score:
            best_score = score
            best_url = entry.get('url')
            
    if best_score > 0.6 and best_url:
        if log_cb: log_cb(f"[green]Найдено на SoundCloud (score: {best_score:.2f})[/green]")
        return f"soundcloud://{best_url}"
        
    if log_cb:
        log_cb(f"[yellow]Нет подходящего совпадения на SoundCloud (лучший score: {best_score:.2f})[/yellow]")
    return None
